...
export function PricingSection() {
  ...
  return (
    <section id="pricing" className="px-6 py-20 bg-[#F9F8F6]">
      <div className="mx-auto max-w-5xl">
        <div className="mb-12 text-center">
          <h2 className="mb-4 text-3xl font-bold text-slate-900">Simple, honest pricing</h2>
            ...
            >
              {plan.highlighted && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                  <span className="rounded-full bg-amber-400 px-3 py-1 text-xs font-semibold text-navy-950">
                    Most popular
                  </span>
                </div>
              )}
              <div className="mb-5">
                ...
                <p
                  className={`mt-1.5 text-sm ${plan.highlighted ? "text-navy-200" : "text-slate-500"}`}
                >
                  {plan.description}
                </p>
              </div>
              ...
              <ul className="mb-6 space-y-2.5">
                {plan.features.map((feature) => (
                  <li key={feature} className="flex items-start gap-2.5">
                    ...
                    />
                    <span
                      className={`text-sm ${plan.highlighted ? "text-navy-100" : "text-slate-600"}`}
                    >
                      {feature}
                    </span>
                  </li>
